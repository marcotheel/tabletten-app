...
function normalizeDate(v){v=String(v||"").trim();if(/^\d{4}-\d{2}-\d{2}$/.test(v))return v;let m=v.match(/^(\d{1,2})[.\-/ ]+(\d{1,2})[.\-/ ]+(\d{4})$/);return m?`${m[3]}-${m[2].padStart(2,"0")}-${m[1].padStart(2,"0")}`:""}
...
function greeting(){const h=new Date().getHours();let g=h<11?"Guten Morgen":h<17?"Guten Tag":"Guten Abend";return profile.name?`${g}, ${profile.name}`:g}
function calc(m){const takes=(m.times?.length||1),dose=Number(m.dosePerTake||1)*takes,days=Math.floor(Number(m.stock||0)/Math.max(dose,.1)),usable=Number(m.stock||0)-Number(m.limit||0),remind=usable<=0?0:Math.floor(usable/Math.max(dose,.1));const reminder=new Date();reminder.setDate(reminder.getDate()+remind);return{days,remind,reminder,daily:dose}}
...
function allDosesToday(){let a=[];meds.forEach(m=>(m.times?.length?m.times:["manuell"]).forEach(t=>a.push([m,t])));return a}
function dueItems(){return allDosesToday().filter(([m,t])=>!taken(m,t))}
function doneCount(){return allDosesToday().length-dueItems().length}
function progressPct(){const all=allDosesToday().length;return all?Math.round(doneCount()/all*100):100}
